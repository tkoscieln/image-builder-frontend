...
test('Compliance step integration test - STIG', async ({ page, cleanup }) => {
  test.setTimeout(120 * 60 * 1000); // 2 hours
  test.skip(
    !isHosted(),
    'Skipping test. Boot test run only on the hosted service.',
  );
  const blueprintName = 'compliance-stig-test-' + uuidv4();
  const policyName = 'test-policy-' + uuidv4();
  const policyType = 'Red Hat STIG for Red Hat Enterprise Linux 10';
  ...
  await test.step('Test packages were installed', async () => {
    const [exitCode, output] = await image.exec(
      'rpm -q firewalld usbguard subscription-manager audit gnutls-utils sudo s-nail \
      libreswan pkcs11-provider rsyslog policycoreutils nss-tools sssd audispd-plugins \
      pcsc-lite opensc openssh-server cronie chrony openssh-clients rsyslog-gnutls \
      policycoreutils-python-utils pcsc-lite-ccid crypto-policies aide fapolicyd',
    );
    expect(exitCode).toBe(0);
    expect(output).toContain('firewalld');
    expect(output).toContain('usbguard');
    expect(output).toContain('subscription-manager');
    expect(output).toContain('audit');
    expect(output).toContain('gnutls-utils');
    expect(output).toContain('sudo');
    expect(output).toContain('s-nail');
    expect(output).toContain('libreswan');
    expect(output).toContain('pkcs11-provider');
    expect(output).toContain('rsyslog');
    expect(output).toContain('policycoreutils');
    expect(output).toContain('nss-tools');
    expect(output).toContain('sssd');
    expect(output).toContain('audispd-plugins');
    expect(output).toContain('pcsc-lite');
    expect(output).toContain('opensc');
    expect(output).toContain('openssh-server');
    expect(output).toContain('cronie');
    expect(output).toContain('chrony');
    expect(output).toContain('openssh-clients');
    expect(output).toContain('rsyslog-gnutls');
    expect(output).toContain('policycoreutils-python-utils');
    expect(output).toContain('pcsc-lite-ccid');
    expect(output).toContain('crypto-policies');
    expect(output).toContain('aide');
    expect(output).toContain('fapolicyd');
  });

  await test.step('Test kernel arguments', async () => {
    const [exitCode, output] = await image.exec('cat /proc/cmdline');
    expect(exitCode).toBe(0);
    expect(output).toContain('pti=on');
  });

  await test.step('Test enabled services', async () => {
    const [pcscdExitCode] = await image.exec('systemctl is-enabled pcscd');
    expect(pcscdExitCode).toBe(0);

    const [auditdExitCode] = await image.exec('systemctl is-enabled auditd');
    expect(auditdExitCode).toBe(0);

    const [sshdExitCode] = await image.exec('systemctl is-enabled sshd');
    expect(sshdExitCode).toBe(0);

    const [fapolicydExitCode] = await image.exec(
      'systemctl is-enabled fapolicyd',
    );
    expect(fapolicydExitCode).toBe(0);

    const [sssdExitCode] = await image.exec('systemctl is-enabled sssd');
    expect(sssdExitCode).toBe(0);
    ...
    const [usbguardExitCode] = await image.exec(
      'systemctl is-enabled usbguard',
    );
    expect(usbguardExitCode).toBe(0);

    const [rsyslogExitCode] = await image.exec('systemctl is-enabled rsyslog');
    expect(rsyslogExitCode).toBe(0);

    const [chronydExitCode] = await image.exec('systemctl is-enabled chronyd');
    expect(chronydExitCode).toBe(0);
  });
  ...
  await test.step('Test disabled services', async () => {
    ...
    const [kdumpExitCode] = await image.exec('systemctl is-enabled kdump');
    expect(kdumpExitCode === 1 || kdumpExitCode === 4).toBeTruthy();

    const [debugShellExitCode] = await image.exec(
      'systemctl is-enabled debug-shell',
    );
    expect(debugShellExitCode === 1 || debugShellExitCode === 4).toBeTruthy();
  });

  await test.step('Test FIPS mode is enabled', async () => {
    const [exitCode, output] = await image.exec(
